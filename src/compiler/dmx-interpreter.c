...
static char *find_range_marker(char *s, int *div_out)
{
    for (char *p = s; *p; p++)
    {
        if (*p != 'x')
...
bool load_dmxd_file(DMXD_File *dmxd)
{
    ...
    while (fgets(dmxd_line, sizeof(dmxd_line), dmxd->handler))
    {
        ...
        int div = 0;
        char *xpos = find_range_marker(keypart, &div);
        if (xpos)
        {
            size_t name_len = (size_t)(xpos - keypart);
            if (name_len >= sizeof(command.name))
                name_len = sizeof(command.name) - 1;
            strncpy(command.name, keypart, name_len);
            command.name[name_len] = 0;
            trim(command.name);
            strip_whitespace(command.name);
...
bool resolve_note(DMXD_File *dmxd, DMX_Instruction *instr, int *note_out)
{
    ...
    for (int i = 0; i < dmxd->command_count; i++)
    {
        ...
        {
            char key[64];
            snprintf(key, sizeof(key), "%s%d-", instr->command, instr->arg1);
